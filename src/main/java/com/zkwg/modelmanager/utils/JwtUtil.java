...
public class JwtUtil {
     ...
     * @return 令牌
     */
    public static String generateToken(AISecurityUser userDetail) {
        Map<String, Object> claims = new HashMap<>(3);
        claims.put(USERID,userDetail.getUserId());
        claims.put(USERNAME, userDetail.getUsername());
        claims.put(CREATED, new Date());
        claims.put(TENANT_ID, userDetail.getTenantId());
     ...
     * @return 用户名
     */
    public AISecurityUser getUserFromToken(HttpServletRequest request) {
        // 获取请求携带的令牌
        ...
        if (StringUtils.isNotEmpty(token)) {
            ...
            Integer tenantId = (Integer)claims.get(TENANT_ID);
            ...
            if (validateToken(token, username)){
                // 未把密码放到jwt
                return new AISecurityUser(userId,tenantId,username,"",authorities,null);
            }
        }
        return null;
    }
